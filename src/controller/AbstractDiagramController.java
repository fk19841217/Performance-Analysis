...
public abstract class AbstractDiagramController {
    ...
    static boolean mouseCreationActivated = true;
    ...
    public AbstractNodeView createNodeView(AbstractNode node, boolean remote) {
        ...
        if (node instanceof ClassNode) {
            ...
            newView = new ActorNodeView((ActorNode) node);
        } else if (node instanceof UsecaseNode) {
            newView = new UsecaseNodeView((UsecaseNode) node);
        } else if (node instanceof NodeNode) {
            newView = new NodeNodeView((NodeNode) node);
        } else if (node instanceof DeploymentNode) {
            newView = new DeploymentNodeView((DeploymentNode) node);
        } else if (node instanceof LinkedDeploymentNode) {
    ...
    void addDiagram(BorderPane diagram) {
        allDiagrams.add(diagram);
    }

    boolean removeDialog(BorderPane diagram) {
        mode = Mode.NO_MODE;
        return allDiagrams.remove(diagram);
    }
