 ...
 */
public class DeploymentController extends AbstractDiagramController {
    ...
    void initDrawPaneActions(){
        drawPane.setOnMousePressed(event -> {
            if (mode == Mode.NO_MODE) {
                ...
                else if ((tool == ToolEnum.CREATE_NODE || tool == ToolEnum.CREATE_DEPLOYMENT) && mouseCreationActivated) { //Start creation of package or class.
                    mode = Mode.CREATING;
        ...
        drawPane.setOnMouseDragged(event -> {
            ...
            else if ((tool == ToolEnum.CREATE_NODE || tool == ToolEnum.CREATE_DEPLOYMENT) && mode == Mode.CREATING && mouseCreationActivated) { //Continue creation of class or package.
        ...
        drawPane.setOnMouseReleased(event -> {
            ...
            if (tool == ToolEnum.CREATE_NODE && mode == Mode.CREATING && mouseCreationActivated) { //Finish creation of class.
                createNodeController.onMouseReleasedNode();
                if (!createNodeController.currentlyCreating()) {
                    mode = Mode.NO_MODE;
                }

            } else if (tool == ToolEnum.CREATE_DEPLOYMENT && mode == Mode.CREATING && mouseCreationActivated) { //Finish creation of package.
                createNodeController.onMouseReleasedDeployment();
                if (!createNodeController.currentlyCreating()) {
                    mode = Mode.NO_MODE;
                }
        ...
        drawPane.setOnTouchPressed(event -> {
            if ((tool == ToolEnum.CREATE_NODE || tool == ToolEnum.CREATE_DEPLOYMENT) && !mouseCreationActivated) {
                mode = Mode.CREATING;
        ...
        drawPane.setOnTouchMoved(event -> {
            if ((tool == ToolEnum.CREATE_NODE || tool == ToolEnum.CREATE_DEPLOYMENT) && mode == Mode.CREATING && !mouseCreationActivated) {
        ...
        drawPane.setOnTouchReleased(event -> {
            if (tool == ToolEnum.CREATE_NODE && mode == Mode.CREATING && !mouseCreationActivated) {
                createNodeController.onTouchReleasedClass(event);
                if (!createNodeController.currentlyCreating()) {
                    mode = Mode.NO_MODE;
                }

            } else if (tool == ToolEnum.CREATE_DEPLOYMENT && mode == Mode.CREATING && !mouseCreationActivated) {
    ...
    void initNodeActions(AbstractNodeView nodeView) {
        ...
        nodeView.setOnMousePressed(event -> {
            ...
            }  else if (tool == ToolEnum.SELECT || tool == ToolEnum.CREATE_NODE) { //Select node
        ...
        nodeView.setOnMouseDragged(event -> {
            if ((tool == ToolEnum.SELECT || tool == ToolEnum.CREATE_NODE) && mode == Mode.DRAGGING) { //Continue dragging selected elements
                ...
                graphController.movePane(event);
            } else if ((tool == ToolEnum.SELECT || tool == ToolEnum.CREATE_NODE) && mode == Mode.RESIZING) { //Continue resizing node.
        ...
        nodeView.setOnMouseReleased(event -> {
            if ((tool == ToolEnum.SELECT || tool == ToolEnum.CREATE_NODE) && mode == Mode.DRAGGING) { //Finish dragging nodes and create a compound command.
                ...
                graphController.movePaneFinished();
                mode = Mode.NO_MODE;
            } else if ((tool == ToolEnum.SELECT || tool == ToolEnum.CREATE_NODE) && mode == Mode.RESIZING) { //Finish resizing node.
        ...
        nodeView.setOnTouchPressed(event -> {
            if (nodeView instanceof PackageNodeView && (tool == ToolEnum.CREATE_NODE || tool == ToolEnum.CREATE_DEPLOYMENT)) {
                mode = Mode.CREATING;
                createNodeController.onTouchPressed(event);
        ...
        nodeView.setOnTouchReleased(event -> {
            if (nodeView instanceof PackageNodeView && tool == ToolEnum.CREATE_NODE && mode == Mode.CREATING) {
                createNodeController.onTouchReleasedClass(event);
                if (!createNodeController.currentlyCreating()) {
                    mode = Mode.NO_MODE;
                }

            } else if (nodeView instanceof PackageNodeView && tool == ToolEnum.CREATE_DEPLOYMENT && mode == Mode.CREATING) {
                createNodeController.onTouchReleasedPackage(event);
                if (!createNodeController.currentlyCreating()) {
                    mode = Mode.NO_MODE;
                }
    ...
    private void initToolBarActions() {
    	 ...
    	 Image image = new Image("/icons/Node.png");
